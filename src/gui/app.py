...
class FolderDrop(TkinterDnD.Tk):
    def __init__(self):
        ...
        # Delete button
        self.delete_button = tk.Button(self.top_frame, text="Delete", command=self.delete_selected_items)
        ...
        self.right_listbox = tk.Listbox(self.middle_frame, selectmode=tk.EXTENDED)
    ...
    def delete_selected_items(self):
        ...
        for index in reversed(left_selection):  # Iterate in reverse to avoid index issues
            selected_item = self.left_listbox.get(index)
    ...
    def open_selected_file_from_left(self, event=None):
        selection = self.left_listbox.curselection()
        for index in selection:
            selected_item = self.left_listbox.get(index)
            full_path = self.left_items_paths.get(selected_item)
            if full_path:
                self.open_file_explorer(full_path)

    def open_selected_file_from_right(self, event=None):
        selection = self.right_listbox.curselection()
        for index in selection:
            selected_item = self.right_listbox.get(index)
            self.open_file_explorer(selected_item)

    def open_selected_file_from_result(self, event=None):
    ...
    def add_to_left_listbox(self, paths):

...
def drop_right(event: tk.Event, listbox: tk.Listbox) -> None:
    ...
    for path in valid_paths:
        # Check if the path is a subfolder of any existing path, ensuring they are on the same drive
        if any(are_paths_on_same_drive(path, existing_path) and os.path.commonpath([path, existing_path]) == existing_path
               for existing_path in existing_paths):
            continue  # Skip adding if the path is a subfolder of an existing path

        # Remove any existing subfolders of the new path
        to_remove: List[str] = [
            existing_path for existing_path in existing_paths
            if are_paths_on_same_drive(path, existing_path) and os.path.commonpath([path, existing_path]) == path
        ]
